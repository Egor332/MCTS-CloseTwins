from src.mcts.config import MCTSConfig
from src.mcts.mcts import MCTS
from src.mcts.strategies.selection import UCTStrategy
from src.mcts.strategies.simulation import RandomSimulation
from src.mcts.strategies.backpropagation import StandardBackprop, SolverBackprop
from src.engine.game import Game

def test_mcts():
    MCTSConfig.USE_SOLVER = True
    
    print("Testing MCTS with Solver enabled...")
    game = Game(alphabet=['A', 'B'], max_length=5)
    mcts = MCTS(
        iterations=100,
        selection=UCTStrategy(),
        simulation=RandomSimulation(),
        backpropagation=SolverBackprop(),
    )
    best_move = mcts.get_best_move(game)
    
    print(f"Best move found: {best_move}")
    
    MCTSConfig.USE_SOLVER = False
    print("Testing MCTS with Solver disabled...")
    mcts_classic = MCTS(
        iterations=100,
        selection=UCTStrategy(),
        simulation=RandomSimulation(),
        backpropagation=StandardBackprop(),
    )
    best_move_classic = mcts_classic.get_best_move(game)
    print(f"Best move found: {best_move_classic}")
    
if __name__ == '__main__':
    test_mcts()
